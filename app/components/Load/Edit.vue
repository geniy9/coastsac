...
const { statesList, loadStatusOptions, categoryOptions, factoringStatusOptions, imageUrl } = useConfig()
...
const fetchBrokers = useDebounceFn(async (q) => {
  try {
    ...
    // const res = await client('/brokers', { query })
...
//     selectedBrokerModel.value = newVal.broker || null
//     state.broker = newVal.broker?.documentId || ''

//     // Очищаем локальные файлы в загрузчиках при смене выбранного груза
...
// Наблюдатель за выбором брокера
...
const onSubmit = async () => {
  ...
  try {
    ...
    const duplicateCheck = await client('/loads', {
      query: { filters: { load_number: { $eq: state.load_number } } }
    })
  ...
  <UModal v-model:open="open">
    <template #content>
      <UForm :state="state" @submit="onSubmit" class="grid gap-6 p-6 overflow-y-auto max-h-[90vh]">
        ...
        <USeparator label="Load Documents" />
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <UploaderFiles 
              ...
              description="Add more JPG, PNG, PDF files" />
            
            <div v-if="existingRateDocs.length" class="text-xs space-y-1 mt-3">
              <p class="font-medium text-gray-500">Current Rate Conf Files:</p>
              <div v-for="(file, idx) in existingRateDocs" :key="file.id" class="flex items-center justify-between bg-muted/50 p-1.5 rounded">
                <a :href="`${imageUrl}${file.url}`" target="_blank" class="text-primary underline">File {{ idx + 1 }}</a>
                <UButton icon="i-lucide-trash" size="xs" color="error" variant="ghost" @click="existingRateDocs.splice(idx, 1)" />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div v-if="existingPodDocs.length" class="text-xs space-y-1 mt-3">
              <p class="font-medium text-gray-500">Current POD/BOL Files:</p>
              <div v-for="(file, idx) in existingPodDocs" :key="file.id" class="flex items-center justify-between bg-muted/50 p-1.5 rounded">
                <a :href="`${imageUrl}${file.url}`" target="_blank" class="text-primary underline">File {{ idx + 1 }}</a>
                <UButton icon="i-lucide-trash" size="xs" color="error" variant="ghost" @click="existingPodDocs.splice(idx, 1)" />
              </div>
            </div>
          </div>
        </div>
        ...
        <template v-if="permissions.isAdmin || permissions.isAccounting">
          ...
          <div class="grid grid-cols-2 gap-4">
            <UFormField label="Factoring Status" name="factoring_status">
              <USelect v-model="state.factoring_status" :items="factoringStatusOptions" class="w-full capitalize" />
            </UFormField>
            <UFormField label="Invoice Amount" name="factoring.invoice_amount">
              <UInput v-model.number="state.factoring.invoice_amount" type="number" step="0.01">
                <template #trailing><div class="text-xs text-muted font-mono">$</div></template>
              </UInput>
            </UFormField>
            ...
            <UFormField label="Advance Received" name="factoring.advance_received">
              <UInput v-model.number="state.factoring.advance_received" type="number" step="0.01">
                <template #trailing><div class="text-xs text-muted font-mono">$</div></template>
              </UInput>
            </UFormField>
            <UFormField label="Factoring Fee" name="factoring.factoring_fee">
              <UInput v-model.number="state.factoring.factoring_fee" type="number" step="0.01">
                <template #trailing><div class="text-xs text-muted font-mono">$</div></template>
              </UInput>
            </UFormField>
            ...
            <UFormField label="Payment Date" name="factoring.payment_date" class="col-span-2">
...
//   try {
//     // 1. Проверяем дубликат
//     // ИСПРАВЛЕНО: query -> params для предотвращения ошибки 500
//     const duplicateCheck = await client('/loads', {
  ...
  <UModal v-model:open="open">
    <template #content>
      <UForm :state="state" @submit="onSubmit" class="grid gap-6 p-6 overflow-y-auto max-h-[90vh]">
        ...
        <div class="grid grid-cols-2 gap-4">
          <!-- Левая колонка: Rate Confirmation -->
          <div>
            <UploaderFiles 
              ref="rateUploaderRef" 
              label="Add Rate Confirmation" 
          ...
          <!-- Правая колонка: POD/BOL с триггером изменения статуса -->
          <div>
            <UploaderFiles 
              ref="podUploaderRef" 
              label="Upload POD / BOL Documents" 
